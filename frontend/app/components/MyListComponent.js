    ...
    const handleSelectTitle = async (item) => {
        setSelectedTitles((prevSelectedTitles) => {
            if (
                ...
                return [
                    ...prevSelectedTitles,
                    { trackid: item._id, title: item.Title },
                ]; // Ensure the same structure is used when adding an item.
            }
        });
    };
    ...
    const renderItem = ({ item }) => {
        ...
        return (
            <>
                ...
                <View
                    key={"separator1-" + item._id.toString()}
                    style={{ height: 1, backgroundColor: "#CCCCCC" }}
                />
                <View
                    key={"separator2-" + item._id.toString()}
                    style={{ height: 1, backgroundColor: "#CCCCCC" }}
                />
            </>
        );
