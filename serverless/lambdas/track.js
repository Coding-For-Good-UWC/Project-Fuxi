...
const updateTrackRating = async (event) => {
  try {
    const { patientId, trackId, rating } = event.body;
    ...
    console.log(
      "NEW RATING: " +
        patient.trackRatings.find((trackRating) => trackRating.track == trackId)
          .rating
    );
...
const getNextTrackId = async (event) => {
  try {
    const { patientId, prevTrackId } = event.body;
    ...
    const era =
      Math.floor(
        (patient.birthdate.getTime() / (1000 * 60 * 60 * 24 * 365) + 18) / 10
      ) *
        10 +
      1960;
    ...
    if (validTracks.length < 15) {
      ...
      const sortedTracks = randomisedTracks.sort(
    ...
    console.log(
      "RATING 2: " +
        patient.trackRatings.find(
          (trackRating) => trackRating.track == trackSelectedId
        ).rating
    );
...
const getTitles = async (event) => {
  const ids = event.query.ids.split(",");
...
const manualYtQuery = async (event) => {
  const query = event.body.searchQuery;
