  ...
  try {
    ...
    const trackRatings = patient.trackRatings.reduce(
      (acc, { track, rating }) => ({
        ...acc,
        [track]:
          acc[track] === -Infinity || rating < 0 ? -Infinity : acc[track] !== undefined ? acc[track] + rating : rating,
      }),
      {},
    );
...
const searchTrack = async (event) => {
  ...
  try {
    ...
    const tracks = await trackModel.find({ Title: title }).skip(skipCount).limit(pageSize).exec();
