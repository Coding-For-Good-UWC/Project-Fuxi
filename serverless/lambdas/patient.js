...
const newPatient = async (event) => {
  try {
    const { name, age, ethnicity, birthdate, birthplace, language, genres, instituteId } = JSON.parse(event.body);
    console.log(name);

    if (!name || !age || !ethnicity || !birthdate || !birthplace || !language || !genres || !instituteId) {
